...
Stealth.prototype.genTransactionProof = function (amount, pubSpendKey, pubViewKey) {
  ...
  const mask = aes256.encrypt(common.bintohex(ECDHSharedSerect.getEncoded(true)), amount.toString());
...
Stealth.prototype.checkTransactionProof = function (txPublicKey, onetimeAddress, mask) {
  ...
  const e = BigInteger.fromBuffer(this.privSpendKey).add(BigInteger.fromBuffer(d)).mod(ecparams.n);
...
Stealth.fromString = function (str) {
  ...
  const payload = decodedPrivacyAddress.slice(0, -8); // payload from 0 to length -8 (each hex = 4 bit)
