...
const amount = 1000000; // 1tomo
const TX_VALUE = '90000000'; // privacy protocol use 1e8 as unit
const trimPrefix = (str, char) => {
    char = char || '0';
    str.replace(new RegExp(`^${char}+`), '');
};

describe('#etetoken #deposit', () => {
    for (let count = 0; count < 1; count++) {
        ...
        it('Successful deposit to to privacy account', (done) => {
            TestUtils.deposit(amount, SENDER_WALLET.privateKey, SENDER_WALLET.address).then((result) => {
                ...
                console.log('isMineUTXO.amount ', isMineUTXO.amount);
                expect(
    ...
    for (let count = 0; count < 0; count++) {
        it('Successful deposit to create decoys', (done) => {
            ...
            TestUtils.deposit(1000000, privateKey, address).then(() => {
                done();
            })
                .catch((err) => {
                    done(err);
