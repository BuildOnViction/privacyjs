...
describe('#etetoken #wallet', () => {
    ...
    describe('#send', () => {
        ...
        it('Should able to send value = 1 UTXO\'s amount ', (done) => {
            const receiver = generateKeys(WALLETS[0].privateKey);
            try {
                sendWallet.send(receiver.pubAddr, '1000000').then((txs) => {
                    expect(txs).to.be.an('array');
                    expect(txs.length).to.be.above(0);
        ...
        it('Should able to send with needed utxos > ring_number', (done) => {
            ...
            try {
                sendWallet.send(receiver.pubAddr, '400000000000000000').then((txs) => {
                    ...
                    expect(
                        receiveMoney.toString(16).toUpperCase() === 'B469471F80140000', // 13000000 in hex
                    );
                    done();
                }).catch((err) => {
                    done(err);
        ...
        it('Should not able to send with amount > balance', (done) => {
            const receiver = generateKeys(WALLETS[0].privateKey);
            try {
                sendWallet.send(receiver.pubAddr, '1000000000000000').then(() => {
                    done(new Error(''));
                }).catch(() => {
                    done();
                });
    ...
    describe('#withdraw', () => {
        ...
        for (let index = 0; index < 5; index++) {
            it('Should able to withdraw', (done) => {
                ...
                try {
                    withdrawWallet.withdraw(receiver, '1000000').then((txs) => {
                        _.each(txs, (NewUTXO) => {
                            expect(NewUTXO).to.not.equal(undefined);
        ...
        it('Should able to withdraw with needed utxos > ring_number', (done) => {
            ...
            const receiver = WALLETS[0].address;
            try {
                withdrawWallet.withdraw(receiver, '8000000').then((txs) => {
                    _.each(txs, (NewUTXO) => {
                        expect(NewUTXO).to.not.equal(undefined);
                        const returnUTXO = NewUTXO.returnValues;
                        const remainUTXOIns = new UTXO(returnUTXO);
        ...
        it('Should not able to send with amount > balance', (done) => {
            const receiver = WALLETS[0].address;
            try {
                withdrawWallet.withdraw(receiver, '10000000000').then(() => {
                    done(new Error(''));
                }).catch(() => {
                    done();
                });
